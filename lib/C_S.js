...
var supportList = {
    ...
    'rpc_server_heartbeat': function() {
        if(CACHE.battle.battleType === 0) {
            // 合作模式
            if(CACHE.autoPlay.cooperateAdvertType && CACHE._times > 0) {
                //return 'server.rpc_server_start_game(2)'; // 合作模式
            } else {
                if(CACHE.autoPlay.pvpAdvertType ) {
                    echo('开始竞技！')
                    return 'server.rpc_server_dragon_ball_switch_deck(1);server.rpc_server_start_game(1)'; // 竞技模式
                }
            }
        }
    },
    ...
    // 视频广告点击观看
    'rpc_server_video_ads_click_watch': function(adType,isCost) {'server.rpc_server_video_ads_click_watch(' + adType + ','+isCost+')'},

    'rpc_server_real_name_authenticate':function(){},
    'rpc_server_dragon_ball_set_deck': function(id,arr){},
};
// 客户端 -> 服务器
