...
var supportList = {
    ...
    'rpc_client_login_return': function (ret) {
        ...
        result += 'server.rpc_server_daily_reward_get_reward(5);'; // 任务视频
        return result;
    },
    /**
     * 开始游戏
    ...
    'rpc_client_daily_reward_info': function(data) {
        ...
        var result = "";
        echo("rpc_client_daily_reward_info",data)
        // 每日奖励 1/2/3/4
        if(!data.includes(1)) {
            echo('[任务] 点击每日任务1');
    ...
    'rpc_client_video_ads_can_watch': function(adType, states) {
        ...
        // 6.球队捐献广告
        // 14. 妙手点金广告
        if (adType === CACHE.AdvertData.miaoshoudianjin && canWatch) {
            echo('[宝箱] 点击支援宝箱');
            // 点击广告
    ...
    'rpc_client_common_reward': function(from, rewardShow, rewardAdd, boxKey) {
        echo("rpc_client_common_reward",from, rewardShow, rewardAdd, boxKey)
