    ...
    'rpc_client_fight_frame_end': function() {
        ...
        if(playInfo.cfg) {
           ...
           var mergeLessBall = function(_keyList){
                if(CACHE.battle.currentRound >800 &&CACHE.battle.MonsterMergeCount<3 &&(CACHE.battle.currentRound%2==0 && (CACHE.battle.MonsterShowTime !=-1) && (Date.now() - CACHE.battle.MonsterShowTime >CACHE.battle.MonsterSafeInterval) && (Date.now() - CACHE.battle.MonsterShowTime <10000))){
                    var evalStr = "";
                    for (i=0; i < _keyList.length ; i++) { // 循环所有球球，寻找可合并的
                        key = _keyList[i];
        ...
        if(CACHE.battle.selfIndex === monsterBaseInfo.side) {
                ...
                CACHE.battle.roundMonsterCount +=1
                    ...
                    var UnmergeBall = function(_keyList) {
                        ...
                        for (i=0; i < _keyList.length ; i++) { // 循环所有球球，寻找可合并的
                            ...
                            if(ballItem) {
                                ...
                                if (mergeFromObj.star>=3 && mergeFromObj.ballType===40){
                                    CACHE.battle.runUnMergeCount+=4;
                                }else{
                       ...
                       // if(CACHE.battle.currentRound >200   && CACHE.battle.roundMonsterCount<9 && ((CACHE.battle.currentRound%2)==1) &&(CACHE.battle.MonsterType < 101 || CACHE.battle.MonsterType > 106)){
                            //echo('match merge')
