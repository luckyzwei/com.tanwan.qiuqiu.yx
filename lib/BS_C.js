        ...
        if(CACHE.battle.selfIndex === monsterBaseInfo.side) {
            ...
            var mergeBall = function(_keyList) {
                ...
                for (i=0; i < _keyList.length ; i++) { // 循环所有球球，寻找可合并的
                    ...
                    if(ballItem) {
                        evalStr += 'battle_server.rpc_server_fight_ball_merge(' + key + ',' + ballItem.ballId + ');';
                        echo('[特殊球球合并] 合并：' + key + ',' + ballItem.ballId);
                        evalStr += 'battle_server.rpc_server_fight_ball_create()'; // 创建球球
                        return evalStr;
                    }
			...
			if(playInfo.cfg) {
				...
				echo("CACHE.battle.currentRound ",CACHE.battle.currentRound ,"CACHE.battle.roundMonsterCount",CACHE.battle.roundMonsterCount,"monsterType",monsterType,"CACHE.battle.currentRound%2",CACHE.battle.currentRound%2)
				if(CACHE.battle.currentRound >200 && CACHE.battle.roundMonsterCount<9 && ((CACHE.battle.currentRound%2)==1) &&(monsterType < 101 || monsterType > 106)){
					echo('match merge')
